# Motion Sticker Pack v0.3.1

发布日期：2026-09-01

v0.3.1 将风格选择器明确分成“核心风格”和 `custom` 长尾两层：核心目录目标为 16 个高区分度方向，但选择器只展示已经通过真实九格证据门槛的条目；未完成同一角色受控对照的候选不会冒充已验证风格。

## 主要更新

- 新增机器可读的核心风格目录与验证状态，目标 16 个，不再把竞品的 36 个名字直接硬编码为可选 preset。
- 保留 `cute` 兼容 ID，并推荐可读别名 `soft-plush`。
- 明确 `custom` 用于文化媒介、复古印刷、UI 和其他长尾需求；自定义风格仍受静图审批、输出合同和制品谱系约束。
- Gallery 文档区分“路线证据”与“同一角色 A/B 风格对照”，避免把不同案例的代表图误读成风格质量排名。
- 记录 16 个候选方向的视觉轴、重叠审计和后续受控验证门槛。
- 静图生成按输入模式选择透明策略：参考图路线使用纯绿 `#00FF00` 源图优先，文字定义路线透明优先；两者都经过像素校验和真实 Alpha 归一化。

## 验证

- 当前选择器继续只暴露已存在的真实九格证据；新增候选在完成静图、真实动作 GIF/WebP、route、processing report 和 manifest lineage 前保持 pending。
- Python 文档、风格 Gallery 和策略检查通过。
- Node：14 项测试通过。
...
本次 v0.3 发布将早期开发分支中的跨格 Alpha 修复并入同一版本，和执行安全、失败不污染交付及可验证制品谱系一起发布。修复了 v0.3 对抗性复测中跨格实例归属漂移造成的 04、07 Alpha 覆盖闪烁拒绝，并为整条生成链补上可验证的制品谱系。
