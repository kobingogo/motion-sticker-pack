...
def main() -> int:
    ...
    release_only_gallery_media = [
        str(path)
        for path in tracked
        if len(path.parts) == 4
        and path.parts[:2] == ("gallery", "styles")
        and path.name in {"motion.webp"}
    ]
    if release_only_gallery_media:
        errors.append(f"release-only gallery media must not be tracked: {release_only_gallery_media[:8]}")
